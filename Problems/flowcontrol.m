...
%% Koch curve
% F{3} = @(u) rot(pi*120/180)*u/3 + repmat([1; 0]*2/3, 1, 2)
%
clc
clear
n = 2;
...
F = {@(u) u/3,...
     @(u) rot(pi*60/180)*u/3 + repmat([1; 0]/3, 1, 2),...
     @(u) rot(-pi*60/180)*u/3 + repmat([0.5; sqrt(3)/6], 1, 2),...
     @(u) u/3 + repmat([1; 0]*2/3, 1, 2)};
S = zeros(2, 2*4^n);

index = zeros(n,4^n);

for k = 1:n
    index(k,:) = reshape(repmat(1:4, 4^(n-k), 4^(k-1)), 1, 4^n);
end
...
for k = 1:(4^n)
    drawLine(S(:,(2*k-1):(2*k)))
end

%% Sierpinsky gasket
clc
n = 7;
F = {@(u) u/2, @(u) u/2 + repmat([1; 0]/2, 1, 3), @(u) u/2 + repmat([1; sqrt(3)]/4, 1, 3)};
%F = {@(u) 2*u/3, @(u) 2*u/3 + repmat([1; 0]/2, 1, 3), @(u) 2*u/3 + repmat([1; sqrt(3)]/4, 1, 3)};
S = zeros(2, 3*3^n);
S0 = [
    0, 1, 1/2;
    0, 0, sqrt(3)/2
...
index = zeros(n,3^n);
for k = 1:n
    index(k,:) = reshape(repmat(1:3, 3^(n-k), 3^(k-1)), 1, 3^n);
end
