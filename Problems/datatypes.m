...
plotOption = {'LineWidth', 2, 'Color', [1, 0.5, 0]};
x = linspace(-5,5);
plot(x, exp(-x.^2), plotOption{:})
...
Country(4).Remarks = {'The largest country.'};
...
%
clc
X = rmfield(Country, {'Member', 'Remarks', 'Remark'});
X(1)


%% Example: Reading your webcam (Image Acquisition Toolbox)
% MATLAB makes a structure to hold information about video devices
% 
clc
videoInfo = imaqhwinfo;
n = length(videoInfo.InstalledAdaptors);
disp('Choose a video adaptor (winvideo, linuxvideo, ...)')
cellfun(@(s, k) disp([num2str(k), '. ', s]),...
    videoInfo.InstalledAdaptors,...
    num2cell(1:n))
adaptorNumber = input(['Choose a number (1--', num2str(n), '): ']);
videoAdaptor = videoInfo.InstalledAdaptors{adaptorNumber};
videoInfo = imaqhwinfo(videoAdaptor);

deviceInfo = videoInfo.DeviceInfo
deviceInfo.SupportedFormats
clc
n = length(videoInfo.DeviceInfo.SupportedFormats);
arrayfun(@(n) disp([num2str(n),'. ', videoInfo.DeviceInfo.SupportedFormats{n}]), 1:n)
res = input('Choose a resolution:');
videoInfo.DeviceInfo.DefaultFormat = videoInfo.DeviceInfo.SupportedFormats{res};
videoObject = eval(videoInfo.DeviceInfo.VideoInputConstructor)
preview(videoObject)
